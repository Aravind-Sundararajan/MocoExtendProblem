...
function OutputData = ReadOpenSimData(dataFileName)
    ...
    OutputData.solverDuration = 0;
    OutputData.objectiveContact = 0;
    OutputData.objectiveControlEffort = 0;
    OutputData.objectiveStateTracking = 0;
    OutputData.objectiveMet = 0;
    OutputData.objectiveAux = 0;
    ...
    for i = 2:1:size(unparsedData.textdata,1)-1
        if(~isempty(unparsedData.textdata{i}))
            ...
            elseif(strcmp(scan{1}(1), 'solver_duration'))
                OutputData.solverDuration = str2double(scan{1}(2));     
            elseif(strcmp(scan{1}(1), 'objective_contact'))
                OutputData.objectiveContact = str2double(scan{1}(2));   
            elseif(strcmp(scan{1}(1), 'objective_control_effort'))
                OutputData.objectiveControlEffort = str2double(scan{1}(2));   
            elseif(strcmp(scan{1}(1), 'objective_state_tracking'))
                OutputData.objectiveStateTracking = str2double(scan{1}(2));   
            elseif(strcmp(scan{1}(1), 'objective_met'))
                OutputData.objectiveMet = str2double(scan{1}(2));   
            elseif(strcmp(scan{1}(1), 'objective_auxiliary_derivatives'))
                OutputData.objectiveAux = str2double(scan{1}(2));  
            elseif(strcmp(scan{1}(1), 'state_track_weight'))
                OutputData.weightStateTracking = str2double(scan{1}(2));
            elseif(strcmp(scan{1}(1), 'contact_track_weight'))   
                OutputData.weightContactTracking = str2double(scan{1}(2));
            end
        end
    end
    ...
    try
        ...
        OutputData.labels = split(unparsedData.textdata{end});
        if isempty(OutputData.labels{end})
            OutputData.labels =   OutputData.labels(1:end-1);
        end
    end     
