...
void MocoCOPGoal::constructProperties() {
    constructProperty_divide_by_displacement(false);
	constructProperty_exponent(2);
}

void MocoCOPGoal::initializeOnModelImpl(const Model& model) const {
    setRequirements(1, 1);
...
void MocoCOPGoal::calcIntegrandImpl(
	...
	for(auto it = out.cop.begin(); it != out.cop.end(); ++it) {//loop over keys i map and update center of pressure
		...
		if (FlattenSpatialVec(out.force[k]).norm() > getModel().get_assembly_accuracy()){
			counter= counter + 1;
    ...
    integrand += m_power_function((base_of_support - mass_center_position).norm());  //mass_center
}

void MocoCOPGoal::calcGoalImpl(
...
SimTK::Matrix MocoCOPGoal::FlattenSpatialVec(const SimTK::SpatialVec& S) const {
